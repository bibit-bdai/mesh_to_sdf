...
from .utils import (
    check_voxels,
    get_raster_points,
    scale_point_cloud_to_unit_sphere,
    scale_to_unit_cube,
...
def get_surface_point_cloud(mesh, surface_point_method='scan', bounding_radius=None, scan_count=100,
                            ...
                            xyzs=None, normals=None):
    if isinstance(mesh, trimesh.Scene):
        mesh = mesh.dump().sum()
    
    if mesh is None:
        assert surface_point_method=='point_cloud', print('Must build from point cloud if not providing mesh.')
        assert xyzs is not None, print('Must provide xyzs if creating surface from point cloud.')
    ...
    if surface_point_method == 'scan':
        ...
        return surface_point_cloud.sample_from_mesh(mesh, sample_point_count=sample_point_count,
                                                    calculate_normals=calculate_normals)
    elif surface_point_method == 'point_cloud':
        return surface_point_cloud.create_from_point_cloud(xyzs, normals)
    else:
